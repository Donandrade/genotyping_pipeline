...
process INDIVIDUAL_CALL {
    tag "$sample | $region"
    // Salva os VCFs individuais (pileups fragmentados)
    ...
    """
    bcftools mpileup ${target_opt} -f ${ref} --annotate FORMAT/AD,FORMAT/DP --min-MQ 20 ${bam} -Oz -o raw.vcf.gz
...
process MERGE_AND_CALL {
    tag "$region"
    // Salva os VCFs finais (Joint Calling)
    ...
    output:
        path "merged.${safe_region}.called.vcf.gz", emit: vcf
...
// ... (Processos QC_BAM e COVERAGE_QC seguem com publishDir para ${params.outdir}/reports) ...
